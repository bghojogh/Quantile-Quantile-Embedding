...
def main():
    ...
    # n_triplets = 500
    n_triplets = 10000
    path_save_images = "./triplets/" + "images/"
    path_save_labels = "./triplets/" + "labels/"
    path_save_tfrecord = "./triplets/" + "tfrecord/"

    # --------------------- read dataset:
    path_dataset = "C:/Users/benya/Desktop/my_PhD/QQE/codes/3_deep_codes/dataset/MNIST_1000/"
    ...
    if do_triplet:
        X_train_classes, y_train_classes = separate_classes(X_train, y_train)
        ...
        make_data_tfrecord(X_train, y_train, path_save_tfrecord, tfrecord_name='data.tfrecords')
...
def make_data_tfrecord(X_train, y_train, path_save_tfrecord, tfrecord_name='data.tfrecords'):
    # X_train: row wise
    if not os.path.exists(path_save_tfrecord):
        os.makedirs(path_save_tfrecord)
    with tf.io.TFRecordWriter(path_save_tfrecord + tfrecord_name) as writer:
        for sample_index in range(X_train.shape[0]):
            if sample_index % 20 == 0:
                print("processing sample " + str(sample_index) + "....")
            # plt.imshow(X_train[sample_index, :].reshape((28, 28)), interpolation='nearest')
            # plt.show()
            save_data_as_tfrecord(X=X_train[sample_index, :], label=y_train[sample_index], writer=writer)
...
def save_data_as_tfrecord(X, label, writer):
    X_bytes = X.tostring()
...
def separate_classes(X_train, y_train):
    ...
    return X_train_classes, y_train_classes
